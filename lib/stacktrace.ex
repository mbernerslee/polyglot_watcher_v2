defmodule PolyglotWatcherV2.Stacktrace do
  def find_files(test_output) do
    find_files([], String.split(test_output, "\n"))
  end

  defp find_files(acc, []) do
    Enum.reverse(acc)
  end

  defp find_files(acc, [line | lines]) do
    case Regex.named_captures(~r/(?<test_file>test\/[a-z\/_\.]+):/, line) do
      %{"test_file" => test_file} ->
        find_files([test_file| acc], lines)

      _ ->
        find_files(acc, lines)
    end
  end
