defmodule PolyglotWatcherV2.Elixir.Cache.MixTestOutputParserTest do
  ...
  describe "run/1" do
    ...
    test "ignores compiler warning footers like '└─ test/foo:722:10'" do
      mix_test_output =
        """
             warning: this clause for run_merge/4 cannot match because a previous clause at line 650 always matches
             │
         722 │     defp run_merge(user, org, mem_from, mem_to) do
             │          ~
             │
             └─ test/dashboard/app/members/merge_test.exs:722:10

        ..........

          1) test something failing (Some.Test)
             test/dashboard/app/members/merge_test.exs:852
             Assertion with == failed
             stacktrace:
               test/dashboard/app/members/merge_test.exs:853: (test)

        Finished in 0.04 seconds (0.00s async, 0.04s sync)
        1 test, 1 failure
        """

      result = MixTestOutputParser.run(mix_test_output)

      assert Map.keys(result) == ["test/dashboard/app/members/merge_test.exs"]
      refute Enum.any?(Map.keys(result), &String.contains?(&1, "└"))
      assert result["test/dashboard/app/members/merge_test.exs"].failure_line_numbers == [852]
    end
