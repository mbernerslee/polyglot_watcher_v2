defmodule PolyglotWatcherV2.Elixir.MixTestLatestTest do
  ...
  describe "failed_max_failures_1/1" do
    test "with a cache hit, runs mix test <test_path> --failed --max-failures 1" do
      server_state = ServerStateBuilder.build()
      test_path = "test/cool_test.exs"
      line_number = 1
      ...
      Mimic.expect(ActionsExecutor, :execute, 3, fn
        {:puts, :magenta, "Running mix test test/cool_test.exs --failed --max-failures 1"},
        server_state ->
          {0, server_state}

        {:mix_test, "test/cool_test.exs --failed --max-failures 1"}, server_state ->
          {0, server_state}
    ...
    test "with a cache hit, runs mix test <test_path> --failed --max-failures 1 and the test fails" do
      server_state = ServerStateBuilder.build()
      test_path = "test/cool_test.exs"
      line_number = 1
    ...
    test "with a cache hit, runs mix test <test_path> --failed --max-failures 1 and the test errors" do
      server_state = ServerStateBuilder.build()
      test_path = "test/cool_test.exs"
      line_number = 1
    ...
    test "with a cache miss return error" do
      ...
      assert {{:cache, :miss}, server_state} == MixTestLatest.failed_max_failures_1(server_state)
    end
  end
end
